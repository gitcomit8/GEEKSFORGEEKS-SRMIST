...
class App {
    constructor(
        ...
        this.update();
        this.addEventListeners(enableGestures);
    }
    createRenderer() {
    ...
    onWheel(e) {
        ...
        // Standard direction: scrolling down (positive delta) moves to next items (positive target)
        const newTarget = this.scroll.target + (delta > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.5;

        // Calculate bounds
        const width = this.medias[0].width;
        const maxScroll = width * (this.medias.length - 1);

        if (newTarget >= 0 && newTarget <= maxScroll) {
            e.preventDefault();
            this.scroll.target = newTarget;
        } else {
            // Allow page scroll if at edges
            this.scroll.target = Math.max(0, Math.min(newTarget, maxScroll));
        }
    ...
    setScrollProgress(progress) {
        ...
        const width = this.medias[0].width;
        const maxScroll = width * (this.medias.length - 1);
        this.scroll.target = maxScroll * progress;
    }
    onResize() {
        this.screen = {
    ...
    addEventListeners(enableGestures) {
...
const CircularGallery = forwardRef(({
    ...
    useEffect(() => {
        const app = new App(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, enableGestures });
        appRef.current = app;
        return () => {
            app.destroy();
            appRef.current = null;
        };
    }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, enableGestures]);
